...
from minicodeagent.context_manager import ContextManager
...
def test_context_manager_assembles_sections_in_expected_order(tmp_path):
    ...
    assert prompt.index("You are MiniCodeAgent") < prompt.index("Memory:")
...
def test_context_manager_relevant_memory_can_mix_durable_notes(tmp_path):
    memory_root = tmp_path / ".minicodeagent" / "memory"
